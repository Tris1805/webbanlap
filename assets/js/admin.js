...
function createAdmin(){
    if(localStorage.getItem('user')==null){
        var user = {username: 'admin', password: 'admin', phone: '0909999999', type: 'ad'};
        userArray.push(user);
...
function createNewUser(){
    ...
    if(document.querySelector('.input-phone').value==""||document.querySelector('.input-phone').value.length<10){
        ...
        var user1 ={username: document.getElementById("user").value, password: document.getElementById('pass').value, phone: document.querySelector(".input-phone").value,type: 'kh'};
...
function changeType(id){
    ...
    var page = '';
    for (i=1;i<=tongsotrang;i++){       
        var a = '<a href="#content" class="page" id="c'+i+'" onclick="renderListChild('+i+','+count+','+tongsotrang+');">'+i+'</a>';
        page+=a;
    }
    page = `<div class="page-body">`+page+`</div>`;
    document.getElementById("pages").innerHTML=page;
